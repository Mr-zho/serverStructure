...
void * manageThreadFunc(void * arg)
{
    ...
    while (true)
    {
        ...
        int aliveThradNums = pool->m_aliveThreadNums;
        ...
        // printf("queueSize = %d, aliveThradNums = %d, busyThreadNums = %d\n", queueSize, aliveThradNums, busyThreadNums);

        /* 什么时候需要开辟新的线程? */
        /* 任务队列任务数 大于 活着的线程数 */
        
        int ret = 0;
        if ((queueSize > (aliveThradNums - busyThreadNums)) && aliveThradNums < pool->m_maxThreadNums)
        {
            int addCnt = 0;
            /* 加锁 */
            pthread_mutex_lock(&(pool->m_mutex));
        ...
        /* 什么时候需要减少线程池的线程的数量 */
        /* 任务队列的任务数 远小于 存活(未工作)的线程数 */
        if (queueSize < aliveThradNums - busyThreadNums && aliveThradNums > pool->m_minThreadNums)
        {
            /* 减少线程 */
            pthread_mutex_lock(&(pool->m_mutex));
